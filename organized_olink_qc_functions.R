...
Multifile_read <- function(directory, file_extension) {
  files_run <- list.files(directory) %>%
    # filter out active excel sheets
    .[!grepl("^~\\$", .)] %>%
    stringr::str_extract(pattern = paste0(".*\\.", file_extension, "$")) %>%
    na.omit() %>%
    as.character()
  # omit empty rows
  if (file_extension == "parquet") {
    ...
    data <-
      lapply(
        X = files_run,
        FUN = function(x) {
          readxl::read_excel(path = x, sheet = 1) %>% dplyr::select(c(4, 6))
        }
      )
  }
  names(data) = str_extract_all(
    string = files_run,
  ...
  if (file_extension == "parquet") {
    lapply(
      ...
      FUN = function(x) {
        arrow::write_parquet(
          data[[x]],
          sink = paste0("SDP/Level_1/", x, ".parquet")
        )
      }
    )
      ...
      FUN = function(x) {
        dplyr::filter(
          ...
          dplyr::mutate(
            ...
            NCLot = NCLot_info,
            AssayTechIssue = NA,
            UniqueID = paste0(PlateID, "-", WellID, "-", SampleID)
          )
      }
...
median_correction <- function(data, Meds) {
  ...
  ref_med <-
    Reduce(function(x, y) dplyr::left_join(x, y, by = "OlinkID"), Meds) %>% # left join all the run median and variance per each run
  ...
  Meds_correction <-
    lapply(Meds, FUN = function(x) dplyr::left_join(x, ref_med)) %>%
  ...
  data_correction <-
    mapply(
      function(x, y) {
        dplyr::left_join(x, y, by = "OlinkID")
      },
      x = data,
      y = Meds_correction,
      SIMPLIFY = FALSE
...
Olink_lvl2_prep <- function(data) {
  data <-
    data %>%
    dplyr::mutate(LogProtExp = ExtNPX_Corrected + log2(1e5)) %>% # transform into LogProExp
    dplyr::mutate(LogProtExp_Raw = ExtNPX_Corrected + log2(1e5)) # transform into LogProExp
...
## Level 2 QC Part 2
Olink_lvl2 <- function(data, multifile = T) {
  ...
  ht_scaled_npx_assay <- data %>%
    ...
    dplyr::mutate(`Below LLOQ` = `Below LLOQ` + `Below LLOD`) %>%
    dplyr::mutate(
...
UMAP_snn <- function(
  ...
  groups <- as.factor(
    sNNclust(dat_umap, k = k, eps = eps, minPts = minPts)$cluster
  )
  ...
  p <- p +
    ...
    geom_text(
      aes(x = xrange[1] + 0.2, y = yrange[1], label = "UMAP1"),
      hjust = 0,
      vjust = 1,
      size = 4
    ) +
    ...
    geom_text(
      aes(x = xrange[1] - 0.6, y = yrange[1], label = "UMAP2"),
      angle = 90,
      hjust = 0,
      vjust = 1,
      size = 4
    ...
    guides(
      colour = guide_legend(
        ncol = 1,
        label.theme = element_text(face = "bold", size = label.size),
...
UMAP_groups <- function(
  ...
  p <- p +
    ...
    geom_text(
      aes(
        x = xrange[1] + (xrange[2] - xrange[1]) * 0.01,
        y = yrange[1],
        label = "UMAP1"
      ),
      hjust = 0,
    ...
    geom_text(
      aes(
        x = xrange[1] - (xrange[2] - xrange[1]) * 0.02,
        y = yrange[1] + (xrange[2] - xrange[1]) * 0.01,
        label = "UMAP2"
      ),
      angle = 90,
      hjust = 0,
